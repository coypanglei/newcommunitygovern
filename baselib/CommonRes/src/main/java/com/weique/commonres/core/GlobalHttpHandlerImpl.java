...
public class GlobalHttpHandlerImpl implements GlobalHttpHandler {
    ...
    public Request onHttpRequestBefore(Interceptor.Chain chain, Request request) {
        ...
        return chain.request().newBuilder().header("token", tokenId)
                              .build(); */
        if (request.url().toString().endsWith(Constants.APK)
                || UserInfoUtils.getInstance().getUserInfo() == null) {
            return request;
        }
        request = request.newBuilder()
                .addHeader(Constants.TOKEN, StringUtil
                        .setText(UserInfoUtils.getInstance().getUserInfo().getToken()))
                .addHeader(Constants.VERSION, AppUtils.getAppVersionName())
                .build();
        return request;
    }
}
