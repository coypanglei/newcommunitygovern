...
public class ArmsUtils {
    ...
    public static int dip2px(float dpValue) {
        final float scale = getResources(AppManager.getAppManager().getApplication()).getDisplayMetrics().density;
        return (int) (dpValue * scale + 0.5f);
    }
    ...
    public  static  int getDrawable(Context context ,String name){
        final int resId = context.getResources().getIdentifier(name, "drawable", context.getPackageName());
        return resId;
    }
    /**
     * findview
    ...
    public static void makeText(String string) {
        ...
        try {
            ...
            mToast = Toast.makeText(AppManager.getAppManager().getApplication(), string, Toast.LENGTH_LONG);
            mToast.setText(string);
            mToast.show();
        } catch (Exception e) {
            e.printStackTrace();
     ...
     */
    @SuppressLint("ShowToast")
    public static void makeTextChildThread(String string) {
        if (TextUtils.isEmpty(string)) {
            return;
        }
        new Thread(new Runnable() {
            ...
            public void run() {
                Looper.prepare();
                Toast.makeText(AppManager.getAppManager().getApplication(), string, Toast.LENGTH_LONG).show();
                Looper.loop();
            }
        }).start();
    }
     ...
     * 获得颜色
     */
    public static int getColor(Context context, String colorName,String packageName) {
        return getColor(context, getResources(context).getIdentifier(colorName, "color", packageName));
    }
